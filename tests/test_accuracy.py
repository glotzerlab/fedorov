...
def test_aflow_prototype():
    ...
    Aflow_data_dir = os.path.join(dir_path,
                                  'crystal_data/Aflow_raw_data.csv')
    ...
    basis_vectors, type_list = structure_test.get_basis_vectors()
    lattice_vectors = structure_test.get_lattice_vectors()
    lattice_vectors_reference = np.array([[2.508, 0, 0],
                                          [-1.254, 2.1719917, 0],
                                          [0, 0, 4.183]])
    basis_vectors_reference = np.array([[0.33333333333333, 0.66666666666667, 0.05995000000000],
    ...
    basis_vectors, type_list = structure_test.get_basis_vectors()
    lattice_vectors = structure_test.get_lattice_vectors()
    basis_vectors_reference = np.array([[0, 0, 0],
                                        [0.5, 0.5, 0.5],
                                        [0.2667, 0.2667, 0.2667],
                                        [-0.2667, -0.2667, -0.2667]])
    a = 6.773648
    alpha = 0.531214
    lattice_vectors_reference = translate_to_vector(a=a, b=a, c=a,
                                                    alpha=alpha, beta=alpha, gamma=alpha)
    assert np.allclose(lattice_vectors, lattice_vectors_reference)
    assert np.allclose(basis_vectors, wrap(basis_vectors_reference))

    # tetragonal and type check
    structure_test = AflowPrototype(prototype_index=5, print_info=False, set_type=True)
    basis_vectors, type_list = structure_test.get_basis_vectors()
    lattice_vectors = structure_test.get_lattice_vectors()
    ...
    basis_vectors_reference = np.array([[0, 0, 0],
                                        ...
                                        [0.5, 0, 0.3785],
                                        [0, 0.5, 0.3785]])
    lattice_vectors_reference = np.array([[4.046, 0, 0],
                                          [0, 4.046, 0],
                                          [0, 0, 4.1394]])
    assert np.allclose(lattice_vectors, lattice_vectors_reference)
    assert np.allclose(basis_vectors, wrap(basis_vectors_reference))
...
def test_prototype():
    structure_test = Prototype(space_group_number=230, wyckoff_site='hh', type_by_site='ac')
    ...
    assert np.allclose(basis_vectors[1, :], wrap(np.array([-0.125, -0.135, -0.145])))
    assert np.allclose(lattice_vectors, np.array([[4, 0, 0], [0, 4, 0], [0, 0, 4]]))
...
def test_fractional_coordinates():
    ...
    assert np.allclose(translate_to_vector(1, 2, 3, 0.5, 0.75, 1.0), lattice_vectors)
...
def test_vector_to_box():
    lattice_vectors = np.array([[1, 0, 0],
                                [1.08060461, 1.68294197, 0],
                                [2.19506661, 1.7193084, 1.10709584]])
    assert np.allclose(lattice_vectors, convert_to_vectors(*convert_to_box(lattice_vectors)))
...
def test_space_group():
    ...
    assert np.allclose(lattice_vectors, np.array([[1, 0, 0], [0, 1, 0], [0, 0, 1]]))
...
def test_prototype_accuracy():
    reference_file = os.path.join(os.path.dirname(__file__), 'structures_for_test.txt')
    ...
    with open(reference_file, 'r') as f:
        for line in f:
            ...
            test_prototype = Prototype(space_group_number=spg_num, wyckoff_site=wyckoff_site)
            basis_vectors, type_list = test_prototype.get_basis_vectors(**struct['basis_params'])
            lattice_vectors = test_prototype.get_lattice_vectors(**struct['lattice_params'])

            assert np.allclose(lattice_vectors, np.asarray(struct['lattice_vectors']))
            pos_comparisons = basis_vectors - \
