...
# use this package unless user would like to abtain the most updated data
# again.  However this code is not maintained and not quaranteed to work if the
# website it queries is changed afterwards.
    ...
    df = df.append(
        {
            "id": (
                ...
                + row["Prototype"]
                + "-"
                + str(row["Space Group Number"])
            ),
            "Pearson_symbol": row["Pearson Symbol"],
            "space_group_number": row["Space Group Number"],
            "Wyckoff_site": extract_info(row["AFLOW Prototype"]),
        },
        ignore_index=True,
    )
...
while i < 590:
    ...
    try:
        ...
        try:
            driver.get(
                "http://aflowlib.org/CrystalDatabase/%s.%s.html"
                % (name, element)
            )
            html = driver.page_source
            soup = BeautifulSoup(html, "html.parser")
...
df.loc[
    208, "basis_params_list"
] = """['x2', 'x3', 'y3', 'x4', 'y4', 'x5', 'y5', 'x6', 'y6', 'x7',
        'y7', 'x8', 'y8', 'x9', 'y9', 'x10', 'y10', 'x11', 'y11',
        'x12', 'y12', 'z12', 'x13', 'y13', 'z13', 'x14', 'y14',
        'z14', 'x15', 'y15', 'z15']"""
df.loc[
    434, "basis_params_list"
] = """['x1', 'x2', 'x3', 'y3', 'x4', 'y4', 'x5', 'y5', 'x6', 'y6',
